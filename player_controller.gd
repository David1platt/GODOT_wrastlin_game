...
func _ready():
	...
	min_health = 0
	fist_g = false
	get_child(2).connect("p_strike", opponent, "on_Enemy_Struck")
	self.connect("move_past", opponent, "_on_Moving_By")
	get_parent().connect("block", opponent, "_on_Block")
...
func _physics_process(delta):
	...
	if collision and collision.collider.get_name() != "ring": #checks if collider is the computer player
